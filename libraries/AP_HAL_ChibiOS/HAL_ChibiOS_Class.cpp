...
#include "hwdef/common/hc32_util.h"
...
static void main_loop()
{
    ...
    if (hc32_was_watchdog_reset()) {
        // load saved watchdog data
        hc32_watchdog_load((uint32_t *)&utilInstance.persistent_data, (sizeof(utilInstance.persistent_data)+3)/4);
    ...
    if (AP_BoardConfig::unlock_flash()) {
        ...
        hc32_flash_protect_flash(false, AP_BoardConfig::protect_flash());
        hc32_flash_protect_flash(true, AP_BoardConfig::protect_bootloader());
    }
#endif
...
static bool Encrypt_code(void){
	uint32_t ID[3]={0,0,0};
	ID[0] = hc32_getUID0();
	ID[1] = hc32_getUID1();
	ID[2] = hc32_getUID2();
    for(int i=0;i<3;i++){
    	if(hc32_flash_read(CHECKADDR+i*4) == 0xFFFFFFFF){
    		ID[i] ^=CHECKWORD;
    		if(!hc32_flash_write(CHECKADDR+i*4,&ID[i],sizeof(ID[i]))){
    			return false;
    		}

    	}
    	else{
    		if((ID[i] ^ (hc32_flash_read(CHECKADDR+i*4))) != CHECKWORD){
    			return false;
    		}
    	}
    }
